...
public class PantallaTareosActualizar extends AppCompatActivity implements View.OnClickListener{
    ...
    String IDTAREO, IDCONSUMIDOR, CONSUMIDOR, IDACTIVIDAD, ACTIVIDAD, IDLABOR, LABOR, IDSUBLABOR, SUBLABOR,  TIPOLABOR, HORAS, IDTURNO;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        actionBar.setTitle("Editar Tareo");

        Bundle parametros = this.getIntent().getExtras();
        IDTAREO = parametros.getString("p_id_tareo");//IDTAREO
        IDCONSUMIDOR = parametros.getString("p_idconsumidor");//CONSUMIDOR
        CONSUMIDOR = parametros.getString("p_consumidor");//IDCONSUMIDOR
        IDACTIVIDAD = parametros.getString("p_idactividad");//ACT
        ACTIVIDAD = parametros.getString("p_actividad");//LABOR
        IDLABOR = parametros.getString("p_idlabor");//FYH
        LABOR = parametros.getString("p_labor");//FYH
        IDSUBLABOR = parametros.getString("p_idsublabor");//FYH
        SUBLABOR = parametros.getString("p_sublabor");//FYH
        TIPOLABOR = parametros.getString("p_tipolabor");//FYH
        HORAS = parametros.getString("p_horas");//FYH
        IDTURNO = parametros.getString("p_idturno");//FYH
...
//        edt_valvula.setOnClickListener(this);
        ...
        id_consumidor_seleccionado=IDCONSUMIDOR;
        edt_consumidor.setText(CONSUMIDOR);
        id_actividad_seleccionado=IDACTIVIDAD;
        edt_actividad.setText(ACTIVIDAD);
        id_labor_seleccionado=IDLABOR;
        edt_labor.setText(LABOR);
        id_sublabor_seleccionado=IDSUBLABOR;
        edt_sublabor.setText(SUBLABOR);
        edt_horas.setText(HORAS);
        if(IDTURNO.equalsIgnoreCase("01")){//DIURNO
            rbDiurno.setChecked(true);
        }else{
            rbNocturno.setChecked(true);
        }
...
//                String observacion = edt_observacion.getText().toString();
                ...
                /*if(!id_actividad_seleccionado.equalsIgnoreCase("A07")){
                    ...
                    if(consumidor.isEmpty() || actividad.isEmpty() || sublabor.isEmpty()){
                        Toast.makeText(PantallaTareosActualizar.this, "Faltan campos por seleccionar!", Toast.LENGTH_SHORT).show();
                    }else{
...
//                }
    ...
    public void grabarTareo(final String id_consu, final String id_act, final String id_lab, final String id_sublab, final String observacion, final String horas, final String idturno){
        ...
        objA.modificarTareoSQLite(IDTAREO, id_consu, id_act, id_lab, id_sublab, "O", horas, idturno);
        Toast.makeText(PantallaTareosActualizar.this, "Tareo ACTUALIZADO CORRECTAMENTE!!", Toast.LENGTH_LONG).show();
        edt_codigo.setText(generarCodigoTareo());
    ...
    public void dialogAdvertenciaGrabar(final String id_consu, final String id_act, final String id_lab){
        android.app.AlertDialog.Builder builder = new android.app.AlertDialog.Builder(PantallaTareosActualizar.this);
    ...
    public void dialogPickerFecha(){
        ...
        DatePickerDialog datePickerDialog = new DatePickerDialog(PantallaTareosActualizar.this,
