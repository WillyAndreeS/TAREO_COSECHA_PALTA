package com.acpagro.tareopalta.PrinterControlBixolon;
...
public class BixolonPrinter implements ErrorListener, OutputCompleteListener, StatusUpdateListener, DirectIOListener, DataListener {
    ...
    public boolean printerOpen(int portType, String logicalName, String address, boolean isAsyncMode) {
        if (setTargetDevice(portType, logicalName, BXLConfigLoader.DEVICE_CATEGORY_POS_PRINTER, address)) {
            try {
                ...
                //posPrinter.setPageModePrintArea("0, 0, 760, 1550");
                mPortType = portType;
                mAddress = address;
    ...
    public boolean printImagePersonality(Bitmap bitmap, int width, int alignment, int brightness) {
        boolean ret = true;
        ...
        try {
            ...
            posPrinter.setPageModePrintArea("0,0,750,1700");
            //posPrinter.setPageModePrintDirection(POSPrinterConst.PTR_BM_LEFT);
            //posPrinter.pageModePrint(POSPrinterConst.PTR_PM_PAGE_MODE);
            //posPrinter.printBitmap(buffer.getInt(0), bitmap, width, alignment);
            posPrinter.printBitmap(buffer.getInt(0), bitmap, width, POSPrinterConst.PTR_BM_LEFT);
            posPrinter.markFeed(0);//Para que se posicione en el siguiente papel de impresion
